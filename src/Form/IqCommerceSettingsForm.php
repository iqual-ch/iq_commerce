...
class IqCommerceSettingsForm extends ConfigFormBase {
  ...
  public static function getIqCommerceSettings() {
    ...
    $iqCommerceSettingsConfig = $language_manager->getLanguageConfigOverride($current_language, 'iq_commerce.settings');
    return [
      'header' => $iqCommerceSettingsConfig->get('header') != NULL ? $iqCommerceSettingsConfig->get('header') : ['value' => '<b>' . t("Commerce Store.") . '</b>', 'format' => 'full_html'],
      'footer' => $iqCommerceSettingsConfig->get('footer') != NULL ? $iqCommerceSettingsConfig->get('footer') : ['value' => t("Thank you for your order. You will receive the item(s) in 3 - 7 days."), 'format' => 'full_html'],
    ];
  }
